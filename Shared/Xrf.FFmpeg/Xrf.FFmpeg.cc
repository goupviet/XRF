...
namespace Xrf
{
	...
	*/
	int FFmpeg::ExtractAllFrames(const char* szPath, const char* szDestination, float scaleFactor)
	{
		// Check if scaleFactor is valid
		if (scaleFactor != 0 && scaleFactor <= 3.0)
		{
			fprintf(stderr, "Xrf: Scale factor out of bounds!\nMust be greater than 0, and less then or equal to 3.0.\n");
			return -1;
		}
